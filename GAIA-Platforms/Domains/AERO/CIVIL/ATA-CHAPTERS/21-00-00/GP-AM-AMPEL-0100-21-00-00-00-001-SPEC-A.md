...
```mermaid
graph TD
    subgraph Inputs & Control
        FMS[Flight Management System]
        ALT[Aircraft Altitude]
        ROC[Rate of Climb/Descent]
        CPC[Cabin Pressure Controllers]
        FMS --> CPC
        ALT --> CPC
        ROC --> CPC
    ...
    subgraph Pressurization Loop
        AA1[Ambient Air] --> EAC[Electric Air Compressors]
        EAC --> PCS[Pressure Control System]
        PCS --> CAB[Cabin Pressure]
        CAB --> OFV[Outflow Valves]
        OFV --> AA2[Ambient Air]
        CPC --> EAC
        CPC --> OFV
    end
    ...
    subgraph Safety Features
        CAB --> SRV[Safety Relief Valves]
        SRV --> AA3[Ambient Air]
        CAB --> NPRV[Negative Pressure Relief Valves]
        NPRV --> AA4[Ambient Air]
    end
```
...
```mermaid
xychart-beta
    title "Cabin Altitude vs. Aircraft Altitude"
    x-axis "Aircraft Altitude (ft)" [0, 10000, 20000, 30000, 40000]
    y-axis "Cabin Altitude (ft)" [0, 2000, 4000, 6000, 8000]
    line "AMPEL360XWLRGA" [
        [0, 0],
        [10000, 0],
        [20000, 2000],
...
```mermaid
xychart-beta
  title "E-ECS Power Consumption Profile"
  x-axis ["Ground Ops", "Takeoff", "Climb", "Cruise", "Descent", "Landing"]
  y-axis "Power (kW)" [0, 50, 100, 150, 200]
  bar [85, 175, 150, 85, 65, 90]
```
